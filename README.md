...
本仓库旨在解决《崩坏：星穹铁道》在 **Intel 显卡（集显/独显）** 环境下使用 Mod 时出现的**人物黑屏、模型爆炸、贴图丢失（半身/透明）**等问题。整合了手动配置文件修复与 Python 哈希自动修正脚本。
...
🚀 修复步骤 (Step-by-Step Guide)
请按照以下顺序进行修复，切勿跳过步骤。

第一步：确保加载器核心正确 (解决全员变黑) 🛑
如果你使用了旧版的 3.6.exe 或其他过期的“修复工具”，会导致人物变成纯黑色剪影。
...
第二步：全局配置修复 (解决衣服乱飞/半身) ⚙️
这是针对 Intel 显卡的最关键步骤。请手动修改 d3dx.ini 配置文件。
